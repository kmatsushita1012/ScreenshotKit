...
- `openURL(.../screenshots/start)` を 1 回送るだけで全 locale × 全 scene を自動巡回
...
- shell スクリプトで `.xcodeproj`、bundle id、URL scheme を自動推定
...
1. アプリ側
   ...
   - `openURL` を受けたら scene を順番に表示して保存する
2. shell 側
   - Simulator を boot する
   - `screenshots/start` を送る
...
shell 側で明示しない場合、`scripts/export_screenshots.sh` は `Info.plist` から最初の URL scheme を推定します。
...
- `Info.plist` から URL scheme を推定する
...
- `output-dir`
  ...
  - 省略時は `./output`
- `url-scheme`
  - URL scheme を手動指定したい場合に使う
  - 省略時は `Info.plist` から推定
- `device-id`
...
`myapp:/screenshots/start?deviceName=...` を受けると、アプリ内では次の順で処理します。

1. `deviceName` を URL から取得
2. セッションディレクトリを作成
...
- URL scheme 推定は `Info.plist` の最初の URL scheme を使います
